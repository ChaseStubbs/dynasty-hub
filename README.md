# CFB27 Dynasty Hub

A single-file dynasty management and content planning tool for **EA Sports College Football 27**.

Built by [JustStubbs](https://www.youtube.com/@JustStubbs) — no installs, no accounts, no server. Just open the file and go.

---

## How to Use

1. Download **dynasty-hub.html** from this repo
2. Open it in any modern browser (Chrome, Firefox, Safari, Edge)
3. Click **⚙ Setup** to enter your team name and pick your team colors
4. That's it — start tracking your dynasty

---

## ⚠️ Back Up Your Data

Your dynasty data is saved in your **browser's local storage** — it stays on your device and never leaves it. This means:

- If you **clear your browser data**, your dynasty is gone
- If you **switch browsers or devices**, your data won't carry over

**Use the 💾 Export button regularly** to save a backup file. Keep it somewhere safe (Desktop, Google Drive, etc.). You can re-import it anytime with the 📂 Import button.

> A good habit: export after every session, and definitely before archiving a season.

---

## Features

- 🎬 Episode Planner — plan and archive recording sessions with auto episode counter
- 📖 Storyline Tracker — manage active, dormant, and resolved narrative arcs
- 🏈 Game Log — log results, season schedule, Heisman watch
- 📊 Season Stats — team offense/defense, position-specific player stats, awards & honors
- 📈 Analysis — This Week's Matchup card, Season Comparison, and Player Comparison tool
- ⭐ Recruiting Board — track targets, NIL spend, class overview and charts; push committed players directly to Dev mid-season
- 🚪 Transfer Portal — separate portal board with NIL tracking and mid-season Dev push
- 🏛 Program / Blueprint — NIL budget, facilities, coaching staff
- 💎 Player Development — roster progression with Prospect Pipeline, source badges, OVR history, NIL demands, class finalization
- 🗂 Depth Chart — 3-deep manual depth chart (Offense / Defense / Special Teams), pulls from Dev roster
- 📰 Universe / News — dynasty news feed and AD goals
- 🎨 Team Colors + Logo — 130+ FBS team color presets, custom logo upload; team color themes the entire UI
- 📋 Season Recap — one-click formatted recap for YouTube descriptions and posts
- 🏆 All-Time Series Stats — career record and totals across all archived seasons
- 📤 Share a Moment — generate shareable game results, player highlights, and recruiting wins for Discord and group chats

---

## Notes

- No dependencies beyond Chart.js (loaded via CDN)
- All data stays in your browser — nothing is sent anywhere
- Single file — everything is in `dynasty-hub.html`
